...
def predcit_simple(
    ...
    # Format input image size
    new_width = input_image.width - input_image.width % 8
    new_height = input_image.height - input_image.height % 8
    input_image = input_image.resize((new_width, new_height), Image.LANCZOS)

    with torch.no_grad():
        if cfg.gen_type == "edge":
            canny = canny_from_pil(
                input_image, cfg.canny_low_threshold, cfg.canny_high_threshold
            )
            c_t = F.to_tensor(canny).unsqueeze(0).to(cfg.device)
            output_image = model(c_t, prompt)

        elif cfg.gen_type == "sketch":
            image_t = F.to_tensor(input_image) < 0.5
            c_t = image_t.unsqueeze(0).to(cfg.device).float()
            ...
            c_t = F.to_tensor(input_image).unsqueeze(0).to(cfg.device)
            output_image = model(c_t, prompt)
...
def predict(args) -> None:
    ...
    with torch.no_grad():
        if args.model_name == "edge_to_image":
            ...
            noise = torch.randn((1, 4, H // 8, W // 8), device=c_t.device)
            output_image = model(
                c_t, args.prompt, deterministic=False, r=args.gamma, noise_map=noise
            )

        else:
            c_t = F.to_tensor(input_image).unsqueeze(0).to("mps")
            output_image = model(c_t, args.prompt)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--low_threshold", type=int, default=100, help="Canny low threshold"
    )
    parser.add_argument(
        "--high_threshold", type=int, default=200, help="Canny high threshold"
    )
    parser.add_argument(
        "--gamma",
        type=float,
